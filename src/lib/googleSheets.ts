...
export async function fetchRunClubsFromSheets(): Promise<RunClub[]> {
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      ...
      range: 'Generated by Outscraper ©!A:Z', // Use the actual sheet tab name and include photo column
    });
    ...
    const runClubs: RunClub[] = dataRows.map((row, index) => {
      ...
      return {
        ...
        full_address: row[4] || '',
        city: row[7] || row[5] || '', // Use city (column 7) first, fallback to borough (column 5)
        postal_code: row[8] || '',
        ...
        photo: row[17] || undefined, // Google Photos URLs in column 17 (R)
        description: row[12] !== 'FALSE' ? row[12] : undefined, // Skip "FALSE" values
        email_1: undefined,               // No email field in current data
        reviews_link: row[15] || undefined, // Google Reviews link in column 15
      };
...
export async function searchRunClubs(query: string = '', location: string = ''): Promise<RunClub[]> {
  ...
  return clubs.filter(club => {
    ...
    const matchesLocation = !location ||
      club.city.toLowerCase() === location.toLowerCase() ||
      club.postal_code.toLowerCase().includes(location.toLowerCase()) ||
      club.full_address.toLowerCase().includes(location.toLowerCase()) ||
      // Handle specific borough mappings to match homepage logic
      (location.toLowerCase() === 'manhattan' && (club.city.toLowerCase() === 'new york' || club.city.toLowerCase().includes('manhattan'))) ||
      (location.toLowerCase() === 'queens' && (club.city.toLowerCase().includes('queens') || club.city.toLowerCase() === 'long island city' || club.city.toLowerCase() === 'jamaica')) ||
      (location.toLowerCase() === 'brooklyn' && (club.city.toLowerCase() === 'brooklyn' || club.city.toLowerCase().includes('brooklyn'))) ||
      (location.toLowerCase() === 'bronx' && (club.city.toLowerCase() === 'bronx' || club.city.toLowerCase().includes('bronx'))) ||
      // General includes match for other locations
      (club.city && club.city.toLowerCase().includes(location.toLowerCase()));
